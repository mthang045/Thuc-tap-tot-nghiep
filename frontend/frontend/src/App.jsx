...
export default function App() {
  ...
  useEffect(() => {
    const restoreSession = async () => {
      try {
        ...
        if (response && response.success && response.user) {
          ...
          try {
            ...
            if (p && p.success && p.profile && p.profile.avatar) {
              setUserAvatar(toAbsoluteUrl(p.profile.avatar));
            }
          } catch (err) {
        ...
        console.log('No active session or session expired', err);
  ...
  useEffect(() => {
    const handler = (ev) => {
      ...
      if (profile && profile.avatar) {
        setUserAvatar(toAbsoluteUrl(profile.avatar));
      } else {
        setUserAvatar('');
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('navigate-pricing', pricingHandler);
    return () => window.removeEventListener('navigate-pricing', pricingHandler);
  }, [navigate]);
    ...
    try {
      ...
      if (response && response.success) {
        ...
        try {
          ...
          console.warn('Could not load profile after Google login:', err);
        }

        return { success: true };
      }

      return { success: false, message: response?.message || response?.error || 'Đăng nhập Google thất bại' };
    } catch (error) {
  ...
  const handleViewAnalysis = (historyItem) => {
    ...
    const analysisData = {
      ...
      issues: issuesArray.map((issue) => ({
        ...
        title: typeof issue === 'string' ? issue.replace(/(?:\u{1F6A8}|\u26A1|\u2139\uFE0F)/gu, '').trim() : (issue.title || issue),
