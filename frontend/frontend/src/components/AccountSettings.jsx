...
export function AccountSettings({ userEmail, onLogout }) {
    ...
    try {
      ...
      if (response.success && response.profile) {
        ...
        if (response.profile.avatar) {
          setAvatarPreview(toAbsoluteUrl(response.profile.avatar));
        }
  ...
  const getStatusBadge = (status) => {
    if (status === 'success') {
      return <span className="px-2 py-1 rounded bg-green-500/20 text-green-300 text-xs">Thành công</span>;
    }
    if (status === 'failed') {
      return <span className="px-2 py-1 rounded bg-red-500/20 text-red-300 text-xs">Thất bại</span>;
    }
    return <span className="px-2 py-1 rounded bg-amber-500/20 text-amber-300 text-xs">Đang xử lý</span>;
  };
          ...
          <div className="bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-xl p-8">
            ...
            {activeTab === 'billing' && (
              <div className="space-y-6">
                ...
                {(() => {
                  const latestSuccess = paymentHistory.find((item) => item.status === 'success');
                  const currentPlan = latestSuccess?.plan || 'free';
                  const currentPrice = latestSuccess?.amount || 0;
                  const currentCycle = latestSuccess?.billing_cycle === 'yearly' ? 'năm' : 'tháng';

                  return (
                    <div className="bg-gradient-to-br from-cyan-900/40 to-blue-900/40 border-2 border-cyan-500/50 rounded-xl p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="text-cyan-100 mb-1">Gói hiện tại: {currentPlan.toUpperCase()}</h3>
                          <p className="text-slate-400 text-sm">Tự động cập nhật sau thanh toán thành công</p>
                        </div>
                        <div className="text-cyan-400 text-2xl">
                          {currentPrice > 0 ? `${new Intl.NumberFormat('vi-VN').format(currentPrice)}đ/${currentCycle}` : '$0'}
                        </div>
                      </div>
                      <a
                        href="#"
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-slate-200">Lịch sử thanh toán</h3>
                    <button
                      onClick={loadPaymentHistory}
                      className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded text-sm"
                      disabled={loadingPayments}
                    >
                      {loadingPayments ? 'Đang tải...' : 'Làm mới'}
                    </button>
                  </div>

                  {paymentHistory.length === 0 ? (
                    <div className="text-slate-400 text-sm">Chưa có giao dịch thanh toán nào.</div>
                  ) : (
                    <div className="space-y-3">
                      {paymentHistory.map((item) => (
                        <div
                          key={item.txn_ref}
                          className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 bg-slate-900/50 border border-slate-700 rounded-lg p-4"
                        >
                          <div className="text-sm">
                            <div className="text-slate-200">Mã GD: {item.txn_ref}</div>
                            <div className="text-slate-400">
                              {item.plan?.toUpperCase()} - {new Intl.NumberFormat('vi-VN').format(item.amount || 0)}đ
                            </div>
