...
function UserEditModal({ user, onClose, onSave, onResetPassword }) {
  const [fullName, setFullName] = useState(user?.full_name || '');
  const [phone, setPhone] = useState(user?.phone || '');
  const [isAdmin, setIsAdmin] = useState(Boolean(user?.is_admin));
  const [subscription, setSubscription] = useState(user?.subscription_tier || 'free');

  useEffect(() => {
    setFullName(user?.full_name || '');
    setPhone(user?.phone || '');
    setIsAdmin(Boolean(user?.is_admin));
    setSubscription(user?.subscription_tier || 'free');
  }, [user]);

  const handleSave = () => {
    onSave({ full_name: fullName, phone, is_admin: isAdmin, subscription_tier: subscription });
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-slate-900 p-6 rounded-lg w-full max-w-lg">
        <h3 className="text-lg text-slate-100 mb-4">Chỉnh sửa người dùng</h3>
        <div className="space-y-3">
          <div>
            <label className="text-sm text-slate-400">Họ và tên</label>
            <input value={fullName} onChange={e => setFullName(e.target.value)} className="w-full p-2 bg-slate-800 rounded mt-1 text-slate-200" />
          </div>
          <div>
            <label className="text-sm text-slate-400">Số điện thoại</label>
            <input value={phone} onChange={e => setPhone(e.target.value)} className="w-full p-2 bg-slate-800 rounded mt-1 text-slate-200" />
          </div>
          <div className="flex items-center gap-3">
            <label className="text-sm text-slate-400">Admin</label>
            <select value={isAdmin ? 'true' : 'false'} onChange={e => setIsAdmin(e.target.value === 'true')} className="p-2 bg-slate-800 rounded text-slate-200">
              <option value="false">False</option>
              <option value="true">True</option>
            </select>
            <label className="text-sm text-slate-400">Gói</label>
            <select value={subscription} onChange={e => setSubscription(e.target.value)} className="p-2 bg-slate-800 rounded text-slate-200">
        ...
        <div className="flex items-center justify-between mt-6">
          <div className="flex gap-2">
            <button onClick={() => onResetPassword()} className="px-3 py-2 bg-amber-600 text-white rounded">Cấp lại mật khẩu mặc định</button>
          </div>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-3 py-2 bg-slate-800 text-slate-200 rounded">Hủy</button>
              ...
              {filteredUsers.map((user, index) => (
                ...
                >
                  ...
                  <td className="py-5 px-6">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        onClick={() => setEditingUser(user.full || user)}
      ...
      {editingUser && (
        <UserEditModal
          ...
          onSave={async (payload) => {
            try {
              const res = await apiService.adminUpdateUser(editingUser._id || editingUser.id, payload);
              if (res && res.success) {
                setToast({ message: 'Cập nhật thành công', type: 'success' });
                setEditingUser(null);
                // reload users
                const r = await apiService.adminListUsers();
                if (r && r.success && r.data) {
                  const mapped = r.data.map(u => ({ id: u._id || u.id, name: u.full_name || u.username || u.email, email: u.email, plan: u.subscription_tier || 'free', status: u.is_active ? 'active' : 'active', joinDate: u.created_at ? new Date(u.created_at).toLocaleDateString('vi-VN') : '', totalAnalyses: u.analysis_count || 0, full: u }));
                  setUsers(mapped);
                }
              }
            } catch (e) {
              setToast({ message: e.message || 'Lỗi', type: 'error' });
            }
          }}
          onResetPassword={async () => {
            try {
              const res = await apiService.adminResetPassword(editingUser._id || editingUser.id);
              if (res && res.success) {
                setToast({ message: 'Đã reset mật khẩu: ' + (res.new_password || 'Abc@123456'), type: 'success' });
              }
            } catch (e) {
              setToast({ message: e.message || 'Lỗi reset mật khẩu', type: 'error' });
            }
          }}
        />
      )}
